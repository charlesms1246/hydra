...
//! payload expires." The first argument is a BLIND, not a nullifier: it stops the
//! commitment being a bare hash of guessable content. Who wrote a message is a signature
//! over this value under a per-author key — `packages/handshake/src/authorship.ts`.
//!
...
fn commitment_binds_both_inputs() {
    ...
    assert(commit(7, 11) != commit(8, 11), 'blind not bound');
...
fn arguments_do_not_commute() {
    // h(a, b) != h(b, a). A commitment that commuted would let a blind be passed off
    // as a content hash.
