...
import { load, save, ingest, clearSpool, summarise, transparencyReport, commitmentNote,
  appealDigest, Reports, type Period } from "./queue.ts";
...
switch (command) {
  ...
  case "report": {
    ...
    const vault = flag("vault");
    // WHY THE FAILURE IS KEPT RATHER THAN SWALLOWED: `catch(() => "")` made "asked and failed"
    // indistinguishable from "did not ask", and the report then told an operator who had passed
    // `--vault` to pass `--vault`. See `commitmentNote`.
    let vaultFailure = "";
    const root = vault
      ? await fetch(`${vault}/v1/pub/root`).then((r) => r.json() as Promise<{ root: string }>)
        .then((r) => r.root).catch((e: Error) => { vaultFailure = e.message; return ""; })
      : "";
    ...
    out("", commitmentNote(root, Boolean(vault), vaultFailure));
