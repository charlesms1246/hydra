...
export const MODERATION_OBSERVABLE: readonly ModerationObservation[] = [
  ...
  },
  {
    ...
    why: "a transparency report is published, so it is public by construction. It is on this table because being public is not the same as being harmless: it discloses the service's operations over time, and at low volume it discloses more than that. A cell of size one against a public timeline anyone can read names the object it refers to and, on the submission surface, plausibly the person behind it — so every cell below the published floor is banded, and the band INCLUDES ZERO, or banding a cell would announce the cell is non-empty. Naming removed PUBLIC ids is a deliberate choice, and its justification was CORRECTED once: it used to read `the on-chain commitment still stands, so a removal anybody can verify against it`, which was written from the channel-message model. A channel message publishes a commitment; A PUBLIC POST DOES NOT TOUCH THE CHAIN AT ALL, so there was no commitment to verify against and the row claimed a mechanism that does not exist. What is actually true is enough: the id was already public — it is how anyone fetched the object — so naming it discloses nothing new about the object, and anybody can check a removal we claim by asking for the id and finding it absent. The cost is unchanged and real: a permanent index of removed content. Encrypted deletions are absent entirely — those are people deleting their own objects, not decisions about them",
  },
];
