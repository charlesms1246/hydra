...
export const MODERATION_OBSERVABLE: readonly ModerationObservation[] = [
  ...
  {
    id: "compelled.removal",
    what: "the id of an encrypted object removed under legal process, the date, and the operator's own reference for the process served — and nothing else, ever",
    why: "THE POSITION HERE CHANGED AND THE OLD REASONING WAS NOT WRONG. It held that an encrypted object the operator can delete on request is one they can be COMPELLED to delete, and that they cannot know what they are deleting — so the class got a capability instead, with no operator discretion in it. That argument still stands; the decision (user, 2026-09-04) is that the ordered, auditable version is worth its cost anyway, because the alternative is not the absence of compelled removal — it is an operator complying off the record with nothing counting it. So this exists to make compulsion COUNTABLE AND VISIBLE rather than easy. Separately keyed from public takedown, so possession of one authority never implies the other and routine moderation cannot escalate into reaching into private messages. One id per request: no channel form, no bulk form, because a bulk path is what gets demanded and a capability acting on one named object is one whose use is countable. NO CLAIM ABOUT CONTENT is recorded or possible — the operator cannot read it, and a field for what it was would eventually hold a guess. The count appears in the transparency report, banded like everything else, so a period with none reads the same as a period with a few. AND THE AFFECTED PARTIES ARE TOLD: a compelled removal answers differently from a miss, so their client can say a message was removed under process rather than expired — a removal indistinguishable from expiry is invisible to the people it happened to, which would make this a backdoor with paperwork",
  },
  {
    id: "appeal.filed",
