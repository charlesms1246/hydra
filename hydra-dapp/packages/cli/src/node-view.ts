...
export const NODE_OBSERVABLE: readonly NodeObservation[] = [
  ...
  },
  {
    ...
    what: "which address a client asked about — that somebody at this IP wanted a named account's published bundle, and when",
    why: "reading a bundle off the identity contract is a `starknet_call` naming the address, so the node learns who you are interested in before you have contacted them. THE ROW EXISTS BECAUSE THIS DISCLOSURE MOVED RATHER THAN APPEARED, and moving it was the point: the alternative is fetching a bundle from the target's VAULT, which tells the party you are considering contacting that you are considering it, at the moment you are still deciding. A node is a party you CHOOSE — you can use one the organisation does not run, change it, or query several addresses at once, none of which is available against their vault. That is a real improvement and not a free one: it is still a link between your address and theirs, held by somebody, and a client that reads one record and then publishes is `node.readTiming` all over again. Padding is cheap here in a way it is not for a public read — record lookups are reads of public data on a contract anyone queries, so asking about several addresses costs nothing but bandwidth",
  },
  {
    id: "node.blockScan",
