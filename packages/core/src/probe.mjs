...
export async function probeIndexer(url) {
  ...
  // OK/1s on its own. So: reachable is `up`, caught-up is `healthy`, and both are reported.
  if (!r.json) return { up: false, reason: r.error ?? `http ${r.status}` };
  return {
    up: true,
    healthy: r.json.status === "OK",
    url,
