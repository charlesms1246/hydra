...
const PROSE = {
  what: [
    ...
    ["k", "cli      the command surface — hydra up, doctor, bootstrap, leak"],
    ["k", "tui      this program"],
  ...
  start: [
    ["h", "Getting started"],
    ["k", "hydra doctor      verify the toolchain — it says how to fix whatever is missing"],
    ["k", "hydra bootstrap   install node dependencies"],
    ["k", "hydra up          devnet + pool + funded accounts + local discovery service"],
    ...
    ["", "API. Leaving them up is right when you are about to run `hydra status` or a"],
  ...
  agents: [
    ...
    ["k", "hydra leak      the disclosure matrix (the Disclosure page)"],
    ["k", "hydra status    the stack (the Overview's stack block)"],
    ["k", "hydra wallets   accounts and balances"],
    ["k", "hydra blocks    recent chain activity"],
    ["k", "hydra doctor    the toolchain rows"],
    ["", ""],
