 ...
 *
 * Every block here is a fixed rectangle sized by `plan()`, so the dashboard is
 * exactly as tall as the terminal and never one row more. Nothing scrolls; a
 * block with more rows than it has room for says how many it dropped, because a
...
const trunc = (s, w) => (String(s ?? "").length <= w ? String(s ?? "") : String(s ?? "").slice(0, Math.max(0, w - 1)) + "…");
 ...
 * than its block, and it has to be here rather than at the call site because
 * every block does it.
 */
const Block = ({ w, h, title, right, rows, tone: titleTone = C.border }) => {
  const inner = Math.max(0, h - 2);
  const shown = rows.slice(0, inner);
  const pad = Array.from({ length: Math.max(0, inner - shown.length) }, (_, i) => i);
 ...
 *
 * Two bands of blocks plus the standing auditor strip. The strip is shed first
 * and the bands shrink after it, but neither band is ever dropped: a dashboard
 * missing its wallets is not a smaller dashboard, it is a broken one.
 */
export function plan(cols, rows, counts = {}) {
  ...
  const wide = cols >= 96;
  // Every band is sized to what it actually has to say, and nothing is inflated
  // to consume the terminal. A 57-row terminal does not contain 57 rows of facts
  // about a three-account devnet, and a block padded to twice its content reads
  // as broken far more than a short frame does. Surplus falls to the bottom.
  const need = (n, min) => Math.max(min, n ?? min) + 2;
  // Stacked, band A carries all three blocks' rows, not the tallest of them.
  // Sizing it as if it were still a row of three is what made it drop six rows
  // and say "+6 more" on an 80-column terminal.
  const aRows = wide ? Math.max(4, counts.tooling ?? 4) : 8 + (counts.tooling ?? 4);
  // Leave band B its minimum (5) and nothing else. Reserving more than that for
  // a strip that is shed anyway is what made the stacked block drop rows it had
  // room for.
  const bandA = Math.min(need(aRows, 4), Math.max(6, rows - 7));
  const bandB = Math.min(need(Math.max(counts.wallets ?? 0, counts.activity ?? 0), 3),
                         Math.max(6, rows - bandA - 5));
  // The strip carries the standing note when the note fits, and one line when it
  // does not. It is never stretched past that.
  const room = rows - bandA - bandB;
  // Below 18 rows the strip is shed entirely: the blocks above it carry facts
  // this one restates, and a truncated standing note is worth less than the rows.
  const strip = rows < 18 ? 0 : room >= 8 ? 7 : room >= 3 ? 3 : 0;
  return { wide, strip, bandA, bandB };
}

export const Overview = ({ cols, rows, svc, wal, blocks, doctor, ledger, control, note }) => {
  const counts = {
    tooling: 1 + (doctor?.rows ?? []).filter((r) => r.status.trim() !== "ok").length,
    wallets: wal?.available ? wal.wallets.length : 1,
    activity: ledger?.length || blocks?.blocks?.length || 1,
  };
  const { wide, strip, bandA, bandB } = plan(cols, rows, counts);
  const cols3 = wide ? [Math.floor(cols / 3), Math.floor(cols / 3), cols - 2 * Math.floor(cols / 3)] : [cols];
  const cols2 = wide ? [Math.floor(cols * 0.46), cols - Math.floor(cols * 0.46)] : [cols];
  ...
  const stackRows = [
    ...
    html`<${Row} label="indexer" w=${cols3[0]} up=${Boolean(ix?.up && ix?.healthy)} warn=${Boolean(ix?.up)}
      value=${ix?.up ? (ix.healthy ? `lag ${ix.lagSecs ?? 0}s` : `lagging ${ix.lagSecs ?? "?"}s`) : ix?.reason ?? "down"} />`,
  ...
  const chainRows = [
    html`<${Row} label="network" w=${cols3[1]} value=${dn?.chainId === "0x534e5f5345504f4c4941" ? "SN_SEPOLIA (devnet)" : dn?.chainId ?? "—"} />`,
  ...
  const toolRows = rowsD.length
    ? [
        ...
        ...rowsD.filter((r) => r.status.trim() !== "ok").slice(0, 3).map((r, i) =>
          html`<${Row} key=${"t" + i} label=${trunc(r.name, 10)} w=${cols3[2]}
            up=${false} warn=${r.status.trim() === "WARN"} value=${r.got} />`),
        missN === 0 && warnN === 0
          ? html`<${Text} color=${C.muted}>${"  every pin and artifact present"}<//>` : null,
      ].filter(Boolean)
  ...
  const walletRows = wal?.available
    ? wal.wallets.map((a, i) => html`
        <${Text} key=${"w" + i} wrap="truncate">
          <${Text} color=${C.accent}>${a.name.padEnd(7)}<//>
          <${Text} color=${C.muted}>${addr(a.address) + "  "}<//>
          <${Text}>${trunc(Object.entries(a.balances ?? {})
            .map(([s, b]) => `${b.formatted ?? "?"} ${s}`).join("  "), Math.max(0, cols2[0] - 25))}<//>
        <//>`)
  ...
  const txRows = ledger?.length
    ? ledger.slice(0, Math.max(1, bandB - 2)).map((r, i) => html`
        <${Text} key=${"x" + i} wrap="truncate">
          <${Text} color=${C.muted}>${(r.at ?? "").padEnd(9)}<//>
          <${Text}>${trunc(r.label ?? r.id ?? "run", Math.max(6, cols2[1] - 26))}<//>
    ...
    : blocks?.available && blocks.blocks?.length
      ? blocks.blocks.slice(0, Math.max(1, bandB - 2)).map((b, i) => html`
          <${Text} key=${"b" + i} wrap="truncate">
            <${Text} color=${C.muted}>${("block " + b.number).padEnd(12)}<//>
            <${Text}>${`${b.txCount} tx`}<//>
          <//>`)
      : [html`<${Text} color=${C.muted}>${"  nothing yet — x runs a flow"}<//>`];

  const band = (widths, blocks_) => html`
    <${Box} flexDirection=${widths.length > 1 ? "row" : "column"}>${blocks_}<//>`;
      ...
      ${band(cols2, wide
        ? [
            html`<${Block} key="w" w=${cols2[0]} h=${bandB} title="wallets"
              right=${wal?.available ? `${wal.wallets.length} accounts` : ""} rows=${walletRows} />`,
          ...
          rows=${[
            ...
            ...(strip >= 7
              ? String(note ?? "").trim().split("\n").map((line, i) =>
                  html`<${Text} key=${"n" + i} color=${C.muted} wrap="truncate">
                    ${"  " + line.trim()}<//>`)
              : []),
          ]} />` : null}
    <//>`;
